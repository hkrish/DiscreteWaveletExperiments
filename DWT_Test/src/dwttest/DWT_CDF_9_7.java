...
public class DWT_CDF_9_7 {
	private static final int	SHIFT		= 12;
	private static final int	ADJUST		= 1 << (SHIFT - 1);

	private static final int	PREDICT_1	= 6497;			// with SHIFT =
																// 12
	private static final int	UPDATE_1	= 217;				// with SHIFT =
																// 12
	private static final int	PREDICT_2	= 3616;			// with SHIFT =
																// 12
	private static final int	UPDATE_2	= 1817;			// with SHIFT =
																// 12
	private static final int	SCALING_1	= 4709;			// with SHIFT =
																// 12
	private static final int	SCALING_2	= 3562;			// with SHIFT =
																// 12
	...
	public DWT_CDF_9_7(int width, int height, int steps) {
		if (width < 8)
			throw new IllegalArgumentException("Invalid transform width (must" + " be at least 8)");

		if (height < 8)
			throw new IllegalArgumentException("Invalid transform width (must" + " be at least 8)");

		if (steps < 2)
			throw new IllegalArgumentException("Invalid number of iterations " + "(must be a least 2)");

		if ((width >> steps) < 8)
			throw new IllegalArgumentException("Invalid width for band L0 (must" + " be at least 8)");

		if ((height >> steps) < 8)
			throw new IllegalArgumentException("Invalid height for band L0 (must" + " be at least 8)");

		if (((width >> steps) << steps) != width)
			throw new IllegalArgumentException("Invalid parameters: change width or number of steps (" + width + " divided by 2^" + steps + " is not an integer value)");

		if (((height >> steps) << steps) != height)
			throw new IllegalArgumentException("Invalid parameters: change height or number of steps (" + height + " divided by 2^" + steps + " is not an integer value)");
